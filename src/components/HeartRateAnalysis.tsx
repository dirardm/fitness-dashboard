...
const HeartRateAnalysis: React.FC<HeartRateAnalysisProps> = ({ data, preview = false }) => {
  ...
  const lineOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `Heart Rate During Workout - ${selectedDate}`,
        font: {
          size: 16,
          weight: 'bold',
        },
    ...
    scales: {
      y: {
        beginAtZero: true,
        //min: Math.min(...dailyData.map(item => item.heartRate)) - 5,
        //max: Math.max(...dailyData.map(item => item.heartRate)) + 5,
        grid: {
